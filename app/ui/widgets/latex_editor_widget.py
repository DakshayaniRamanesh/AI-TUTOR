...
class LatexEditorWidget(QWidget):
    ...
    def set_latex_code(self, code: str, title: str = "LaTeX Document", pdf_path: str = ""):
    ...
    def recompile_preview(self):
        ...
        # Auto-repair unclosed environments and markdown formatting before compiling
        try:
            from ...backend.math_engine.latex_client import repair_latex_document
            repaired_code = repair_latex_document(code)
            if repaired_code and repaired_code != code:
                cursor = self.editor.textCursor()
                pos = cursor.position()
                self.editor.setPlainText(repaired_code)
                cursor.setPosition(min(pos, len(repaired_code)))
