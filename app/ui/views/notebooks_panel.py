...
Supports nested folder structure, Rename/Move/Delete context menus per card.
"""
...
class NotebookRowWidget(QFrame):
    ...
    delete_clicked = pyqtSignal(str, str)    # notebook_id, name
    ...
    def __init__(self, data: dict, parent=None):
        ...
        self.setStyleSheet("""
            ...
            QPushButton#BtnDel:hover { color: #d32f2f; background-color: #ffebee; border-radius: 6px; }
            QPushButton#BtnMenu {
                background: transparent; border: none; font-size: 16px;
                color: #8e8e93; padding: 4px; border-radius: 6px;
            }
            QPushButton#BtnMenu:hover { background-color: #e5e5ea; color: #1c1c1e; }
        """)
    ...
    def _show_context_menu(self):
        ...
        if action == act_rename:
            self.rename_requested.emit(self.notebook_id)
        elif action == act_move:
            self.move_requested.emit(self.notebook_id)
        elif action == act_delete:
            self.delete_clicked.emit(self.notebook_id, self.notebook_name)
